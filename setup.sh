...
# A launcher, not a symlink: 'ln -s' silently COPIES on Git Bash and other
# MSYS environments, which leaves a stale duplicate that ignores git pull.
# Running the script in place means an update always takes effect.
...
cat > "$LINK" <<LAUNCHER
#!/bin/sh
# Generated by setup.sh - runs the checkout in place so 'git pull' applies.
